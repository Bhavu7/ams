...
if (isset($_POST['submit_attendance'])) {
    if (isset($_POST['attendance'])) {
        // Extract attendance data from the form submission
        $attendance_data = $_POST['attendance'];
        $attendance_date = date("Y-m-d"); // Current date for attendance

        // Loop through each student to insert their attendance record
        foreach ($attendance_data as $roll_number => $data) {
            // Get student information based on roll number
            $status = $data['status'] ?? 'absent';

            // Query to fetch student details from student_mst table
            $student_sql = "SELECT roll_number, name, department, division, semester, photo FROM student_mst WHERE roll_number = '$roll_number'";
            $student_result = $conn->query($student_sql);

            if ($student_result && $student_result->num_rows > 0) {
                $student = $student_result->fetch_assoc();

                // Insert data into attendance_mst table
                ...
                // Process the query and add to message based on success or failure
        ...
        // Display a single alert with all messages
        echo "<script>
            alert('$message');
            window.location = window.location.href;  // Refresh the page to stay on the current section
        </script>";
    } else {
        echo "<script>alert('No attendance data provided!');</script>";
    }
}
...
<head>
    ...
    <script>
        function downloadReport() {
            ...
            // Check if all filters have values selected
            ...
            rows.forEach((row, index) => {
                if (index > 0) { // Skip header row
                    ...
                    const rowData = [
                        cells[1].innerText, // Roll Number
                        cells[2].innerText, // Student ID
                        cells[3].innerText, // Name
                        cells[4].innerText, // Department
                        cells[5].innerText, // Division
                        cells[6].innerText, // Semester
                        cells[7].innerText, // Attendance Date
                        cells[8].querySelector('input').checked ? 'Present' : '', // Present status
                        cells[9].querySelector('input').checked ? 'Absent' : '', // Absent status
                    ];
            ...
            doc.autoTable({
                head: [
                    ['Roll Number', 'Student ID', 'Name', 'Department', 'Division', 'Semester', 'Attendance Date', 'Present', 'Absent']
                ],
                body: tableData,
                startY: 30,
                theme: 'grid',
                styles: {
                    fillColor: [255, 255, 255], // White background (transparent)
                    textColor: [0, 0, 0], // Black text
                    fontSize: 6, // Default font size
                    lineWidth: 0.2, // Remove borders
                    lineColor: [0, 0, 0], // Remove lines (set border color to white)
                },
                headStyles: {
                ...
                bodyStyles: {
                    lineColor: [0, 0, 0], // Remove borders
                    lineWidth: 0.2,
                },
                didDrawCell: function(data) {
                    // Handling color changes based on Present or Absent status
                    if (data.column.index === 7 || data.column.index === 8) { // Present or Absent columns
                        const isPresent = data.row.raw[7] === 'Present';
                        const isAbsent = data.row.raw[8] === 'Absent';

                        // Apply light green for present rows and light red for absent rows
                        if (isPresent) {
                ...
                columnStyles: {
                    0: { // First column (Roll Number)
                        cellWidth: 15,
                    },
                    1: { // Second column (Student ID)
                        cellWidth: 15,
                    },
                    2: { // Third column (Name)
                        cellWidth: 40,
            ...
            const filename = `Attendance_Report_${department}_${division}_${semester}_${currentDate.toISOString().split('T')[0]}.pdf`;
            doc.save(filename);
        }
...
<body>
                            ...
                            if (isset($_POST['department']) && isset($_POST['division']) && isset($_POST['semester'])) {
                                ...
                                if ($result === false) {
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        <select name='attendance[" . $row['roll_number'] . "][status]' 
                                    ...
                                    echo "<tr><td colspan='10' class='px-6 py-4 text-center text-gray-500'>No students found</td></tr>";
                                }
                            }
                            ?>
                        </tbody>
    ...
    <!-- Inserting Records using excel file -->
    <script>
